...
// Inisialisasi list peminjaman
void initPeminjamanList(PeminjamanList* list) {
    list->head = NULL;
    list->size = 0;
}

// Menambahkan peminjam ke dalam antrian prioritas
...
// Proses peminjaman buku (memberikan buku ke peminjam prioritas tertinggi)
...
void batalkanPeminjaman(PeminjamanList* list, const char* judulBuku, const char* idAnggota) {
    ...
    int found = 0;
    PeminjamanNode** heads[] = {
        &(currentListNode->data.headDosen),
        &(currentListNode->data.headMahasiswa),
        &(currentListNode->data.headMasyarakat)
    };
    
    int i = 0;
    while (i < 3 && !found) {
        PeminjamanNode* current = *heads[i];
...
// Membebaskan memori yang digunakan oleh list peminjaman
